...
/// Bulk-materializes a column into ternary representation for SIMD-friendly processing.
/// This replaces the per-element std::function approach with a single pass over the column,
/// enabling the compiler to auto-vectorize the subsequent combine loops.
...
{
    static void build(const IColumn * x, UInt8 * __restrict ternary_data)
    {
        ...
        else if (const auto * nullable_column = typeid_cast<const ColumnNullable *>(x))
        {
            if (const auto * nested_column = typeid_cast<const ColumnVector<Type> *>(nullable_column->getNestedColumnPtr().get()))
            {
                ...
                for (size_t i = 0; i < size; ++i)
                    ternary_data[i] = Ternary::makeValue(column_data[i], null_data[i]);
            }
            else
                TernaryValueBuilderImpl<Types...>::build(x, ternary_data);
        }
        else if (const auto * column = typeid_cast<const ColumnVector<Type> *>(x))
        {
            const auto & column_data = column->getData();
            for (size_t i = 0; i < size; ++i)
                ternary_data[i] = Ternary::makeValue(column_data[i]);
        }
        else
            TernaryValueBuilderImpl<Types...>::build(x, ternary_data);
    }
...
struct TernaryValueBuilderImpl<>
{
    [[noreturn]] static void build(const IColumn * x, UInt8 *)
    {
        throw Exception(
...
/// Vectorized AssociativeGenericApplierImpl: materializes each column to a UInt8 buffer
/// then combines with simple bitwise ops that auto-vectorize (AND=&, OR=|).
template <typename Op, size_t N>
